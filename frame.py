            ...
            for item in left_navs_dict['N']:
                if item.section_name is not None:
                    with ui.link(target = item.uri).classes("w-full"):
                        ui.button(item.section_name,icon="home" if item.section_icon is None else item.section_icon).props("flat").classes("w-full justify-center text-blue-grey-10 text-h6")
                else:
                    ui.separator()
